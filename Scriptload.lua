...
local __localPlayer = __players.LocalPlayer or __players.PlayerAdded:Wait()
local __character = __localPlayer.Character or __localPlayer.CharacterAdded:Wait()
__character:WaitForChild("Humanoid", 30)
__character:WaitForChild("HumanoidRootPart", 30)
__localPlayer:WaitForChild("Data", 30)
game:GetService("ReplicatedStorage"):WaitForChild("Remotes", 30)

local __workerQueue = {}
local __workersStarted = false
local function __runWorker(worker)
    local ok, failure = xpcall(worker, function(message)
        return debug.traceback(tostring(message), 2)
    end)
    if not ok then
        warn("Maru worker stopped safely: " .. tostring(failure))
    end
end

local function __stableSpawn(worker)
    if type(worker) ~= "function" then
        return nil
    end
    if __workersStarted then
        return task.spawn(__runWorker, worker)
    end
    table.insert(__workerQueue, worker)
    return worker
end

local function __startStableWorkers()
    __workersStarted = true
    for index, worker in ipairs(__workerQueue) do
        task.delay((index - 1) * 0.08, __runWorker, worker)
    end
    table.clear(__workerQueue)
end
...
local Fluent = (function()
    ...
    local COLORS = {
        ...
        SurfaceHover = Color3.fromRGB(34, 37, 47),
        Accent = Color3.fromRGB(73, 170, 255),
        AccentDark = Color3.fromRGB(39, 106, 170),
        Text = Color3.fromRGB(236, 239, 245),
        Muted = Color3.fromRGB(150, 158, 175),
        Stroke = Color3.fromRGB(53, 57, 70),
        Success = Color3.fromRGB(82, 205, 140)
    }
    ...
    local function safeCallback(callback, ...)
        ...
        task.spawn(function()
            ...
            if not ok then
                warn("Maru UI callback: " .. tostring(failure))
            end
        end)
    end
    ...
    local function makeBaseElement(tab, height)
        ...
        tab.NextOrder = tab.NextOrder + 1
        corner(frame, 7)
        stroke(frame)
        return frame
    end

    local function makeTitle(parent, text, y, height)
        return create("TextLabel", {
            Name = "Title",
            Position = UDim2.fromOffset(12, y or 0),
            Size = UDim2.new(1, -24, 0, height or 22),
    ...
    local function makeDescription(parent, text, y, height)
        return create("TextLabel", {
            Name = "Description",
            Position = UDim2.fromOffset(12, y or 23),
            Size = UDim2.new(1, -24, 0, height or 18),
            BackgroundTransparency = 1,
            Text = tostring(text or ""),
    ...
    local function createValueObject(id, defaultValue)
        local object = {
            Id = id,
            Value = defaultValue,
            Changed = {}
        ...
        function object:SetDesc(text)
            if self.DescriptionLabel then
                self.DescriptionLabel.Text = tostring(text or "")
            elseif self.ValueLabel then
                self.ValueLabel.Text = tostring(text or "")
            end
            return self
        end
        if id then
            Library.Options[id] = object
        end
        return object
    end
    ...
    function TabMethods:AddParagraph(config)
        ...
        local frame = makeBaseElement(self, content ~= "" and 64 or 46)
        makeTitle(frame, config.Title or "Paragraph", 7, 21)
        local description = makeDescription(frame, content, 29, 28)
        local object = createValueObject(nil, content)
        object.Frame = frame
    ...
    function TabMethods:AddButton(config)
        ...
        local frame = makeBaseElement(self, description ~= "" and 62 or 46)
        local title = makeTitle(frame, config.Title or "Button", description ~= "" and 7 or 12, 21)
        if description ~= "" then
            makeDescription(frame, description, 29, 20)
        end
        ...
        hitbox.MouseEnter:Connect(function()
            TweenService:Create(frame, TweenInfo.new(0.12), {BackgroundColor3 = COLORS.SurfaceHover}):Play()
        end)
        hitbox.MouseLeave:Connect(function()
            TweenService:Create(frame, TweenInfo.new(0.12), {BackgroundColor3 = COLORS.Surface}):Play()
        end)
        hitbox.MouseButton1Click:Connect(function()
            safeCallback(config.Callback)
        end)
        return {Frame = frame, TitleLabel = title}
    end

    function TabMethods:AddToggle(id, config)
        config = config or {}
        local object = createValueObject(id, config.Default == true)
        local frame = makeBaseElement(self, 48)
        object.Frame = frame
        makeTitle(frame, config.Title or id or "Toggle", 13, 22)
        ...
        local function render(value, instant)
            local trackColor = value and COLORS.Accent or COLORS.Stroke
            local knobPosition = value and UDim2.new(1, -20, 0.5, 0) or UDim2.new(0, 3, 0.5, 0)
            if instant then
                track.BackgroundColor3 = trackColor
                knob.Position = knobPosition
            else
                TweenService:Create(track, TweenInfo.new(0.14), {BackgroundColor3 = trackColor}):Play()
                TweenService:Create(knob, TweenInfo.new(0.14), {Position = knobPosition}):Play()
            end
        end
        ...
        render(object.Value, true)
        return object
    end

    function TabMethods:AddInput(id, config)
        config = config or {}
        local object = createValueObject(id, tostring(config.Default or ""))
        local frame = makeBaseElement(self, 66)
        object.Frame = frame
        makeTitle(frame, config.Title or id or "Input", 7, 20)
        ...
        textbox.FocusLost:Connect(function(enterPressed)
            if config.Numeric and tonumber(textbox.Text) == nil then
                textbox.Text = tostring(object.Value or "")
                return
            end
            if config.Finished == false or enterPressed then
    ...
    function TabMethods:AddDropdown(id, config)
        config = config or {}
        ...
        local object = createValueObject(id, default)
        object.Values = values
        local frame = makeBaseElement(self, 62)
        object.Frame = frame
        makeTitle(frame, config.Title or id or "Dropdown", 6, 20)
        ...
        button.MouseButton1Click:Connect(function()
            ...
            local currentIndex = table.find(object.Values, object.Value) or 0
            local nextIndex = currentIndex % #object.Values + 1
            object:SetValue(object.Values[nextIndex])
        end)
        return object
    end

    function TabMethods:AddSlider(id, config)
        config = config or {}
        ...
        local default = math.clamp(tonumber(config.Default) or minimum, minimum, maximum)
        local object = createValueObject(id, default)
        local frame = makeBaseElement(self, 66)
        object.Frame = frame
        makeTitle(frame, config.Title or id or "Slider", 7, 20)
        ...
        corner(plus, 6)
        local step = rounding > 0 and 10 ^ (-rounding) or 1
        function object:SetValue(value)
        ...
        minus.MouseButton1Click:Connect(function()
            object:SetValue(object.Value - step)
        end)
    ...
    function WindowMethods:AddTab(config)
        ...
        corner(button, 7)
        create("UIPadding", {PaddingLeft = UDim.new(0, 12), Parent = button})
        local content = create("ScrollingFrame", {
            Name = "Content_" .. tostring(index),
    ...
    function WindowMethods:SelectTab(index)
        ...
        for tabIndex, tab in ipairs(self.Tabs) do
            ...
            tab.Button.BackgroundColor3 = selected and COLORS.AccentDark or COLORS.Surface
            tab.Button.TextColor3 = selected and COLORS.Text or COLORS.Muted
        end
    ...
    function Library:CreateWindow(config)
        ...
        local width = math.min(requestedSize.X.Offset > 0 and requestedSize.X.Offset or 620, math.max(340, viewport.X - 24))
        local height = math.min(requestedSize.Y.Offset > 0 and requestedSize.Y.Offset or 420, math.max(280, viewport.Y - 50))
        ...
        create("TextLabel", {
            ...
            Size = UDim2.new(1, -70, 0, 21),
            BackgroundTransparency = 1,
            Text = tostring(config.Title or "Maru Hub"),
            TextColor3 = COLORS.Text,
            TextSize = 16,
            Font = Enum.Font.GothamBold,
            TextXAlignment = Enum.TextXAlignment.Left,
        ...
        local tabWidth = math.clamp(tonumber(config.TabWidth) or 155, 120, math.max(120, width - 200))
        local tabList = create("ScrollingFrame", {
            Name = "Tabs",
            Position = UDim2.fromOffset(0, 46),
            Size = UDim2.new(0, tabWidth, 1, -46),
            BackgroundColor3 = COLORS.Surface,
            BorderSizePixel = 0,
            CanvasSize = UDim2.fromOffset(0, 0),
        ...
        bindCanvas(tabList, tabLayout)
        create("Frame", {
            Position = UDim2.fromOffset(tabWidth, 46),
    ...
    function Library:Notify(config)
        ...
        corner(toast, 8)
        stroke(toast, COLORS.Accent)
        makeTitle(toast, config.Title or "Maru Hub", 8, 20)
        makeDescription(toast, config.Content or config.SubContent or "", 31, 31)
...
do
    ...
    elseif id == 7449423635 then
        ...
        warn("Maru Stable: unsupported PlaceId " .. tostring(game.PlaceId))
        return
    end
...
ToggleButton.MouseButton1Click:Connect(function()
    ...
    -- aimbot mastery hook disabled by stability patch
    --Equip Gun
    __stableSpawn(
        function()
            pcall(
                        ...
                        pcall(
                            function()
                                if CheckPirateBoat() then
                                    ...
                                    repeat
                                        wait()
                                        __stableSpawn(Tween(v.Engine.CFrame * CFrame.new(0, -20, 0)), 1)
                                        AimBotSkillPosition =
...
__stableSpawn(
    function()
        ...
        -- PvP __namecall hook disabled by stability patch
...
do
    ...
    getgenv().rejoin = promptOverlay.ChildAdded:Connect(function(child)
        if _G.AutoRejoin
            and child.Name == "ErrorPrompt"
...
local NoRock = Tabs.S:AddToggle("NoRock", {Title = "No Clip Ship", Default = false })
...
-- Sea-skill __namecall hook disabled by stability patch
