-- Stability patch: startup synchronization, single-instance guard, and safe pacing.
local __maruEnv = (getgenv and getgenv()) or _G
if __maruEnv.__MARU_STABLE_RUNNING then
    warn("Maru Stable is already running; duplicate launch blocked.")
    return
end
__maruEnv.__MARU_STABLE_RUNNING = true
  ...
  for _, url in ipairs(urls) do
    ...
    if ok and type(body) == "string" and #body > 1000 and not body:find("^%s*<") then
      ...
      if chunk then
        ...
        if ran and type(lib) == "table" then vu1 = lib break end
        lastErr = "lib returned " .. type(lib)
      else
        lastErr = "compile: " .. tostring(err)
      end
    elseif ok then
      lastErr = "empty/invalid body (" .. tostring(#body) .. " bytes)"
    else
      lastErr = tostring(body)
    end
  end
  if not vu1 then
    warn("Banana Pro: UI library failed to load - " .. tostring(lastErr))
  end
end
if not vu1 then return end
...
elseif v5 == 7449423635 then
    ...
    __maruEnv.__MARU_STABLE_RUNNING = false
    warn("Maru Stable: unsupported PlaceId " .. tostring(v5))
    return
end
...
-- ===== Floating toggle button (Banana Pro rebuild) =====
-- Bản gốc bị hỏng do artifact của decompiler (vu234/vu232/vu235 và global Window
-- không bao giờ được định nghĩa) -> script chết ngay trước khi dựng menu.
-- Dựng lại sạch, an toàn, dùng đúng biến window v2.
pcall(function()
    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "BananaPro_ControlGUI"
...
-- Dọn effect an toàn (guard chống nil khi Effect/Container chưa load)
pcall(function()
...
-- ===== Player Tools + Server Tools (Banana Pro nâng cấp) =====
pcall(function()
    local P = game:GetService("Players")
    local lp = P.LocalPlayer
    local plrDrop, plrSig = nil, ""
    local function refreshPlrDrop()
        local names = {}
        ...
        if sig ~= plrSig then
            plrSig = sig
            if plrDrop then
                pcall(function() plrDrop:SetValues(names) end)
            end
        end
    end
    plrDrop = v3.Home:AddDropdown("DropdownBananaPlayer", {
        ["Title"] = "Chọn Người Chơi",
        ["Values"] = {},
        ["Callback"] = function(v) _G.Banana_SelectedPlayer = v end
    })
    local function selectedRoot()
        local t = _G.Banana_SelectedPlayer and P:FindFirstChild(_G.Banana_SelectedPlayer)
    ...
    v3.Home:AddButton({
        ...
        ["Callback"] = function()
            pcall(function()
                local r = selectedRoot()
                if r then
                    local hrp = lp.Character and lp.Character:FindFirstChild("HumanoidRootPart")
                    if hrp then hrp.CFrame = r.CFrame * CFrame.new(0, 5, 0) end
                end
            end)
        end
    })
    v3.Home:AddToggle("ToggleBananaSpectate", {
        ["Title"] = "Spectate Người Chơi",
        ["Callback"] = function(v)
            _G.Banana_Spectate = v
            if not v then
                pcall(function()
    ...
    task.spawn(function()
        while task.wait(0.2) do
            if _G.Banana_Spectate then
                pcall(function()
                    local r = selectedRoot()
                    local h = r and r.Parent and r.Parent:FindFirstChildOfClass("Humanoid")
                    if h then workspace.CurrentCamera.CameraSubject = h end
                end)
            end
        end
    ...
    v3.Home:AddButton({
        ...
        ["Callback"] = function()
            pcall(function()
                ...
                local req = (syn and syn.request) or (http_request and http_request)
                if not req then
                ...
                for _, v in pairs(data.data or {}) do
                    if v.id ~= game.JobId and v.playing and v.maxPlayers and v.playing < v.maxPlayers then
    ...
    v3.Home:AddButton({
        ...
        ["Callback"] = function()
            pcall(function()
                setclipboard("https://www.roblox.com/games/" .. game.PlaceId .. "/server/" .. game.JobId)
            end)
        end
    })
    refreshPlrDrop()
    P.PlayerAdded:Connect(refreshPlrDrop)
    P.PlayerRemoving:Connect(refreshPlrDrop)
    task.spawn(function()
        while task.wait(0.5) do
            pcall(refreshPlrDrop)
        end
    end)
end)
...
do
    ...
    if __maruEnv.rejoin then
        __maruEnv.rejoin:Disconnect()
    end
    __maruEnv.rejoin = promptOverlay.ChildAdded:Connect(function(p1004)
        if _G.AutoRejoin
            and p1004.Name == "ErrorPrompt"
            and p1004:FindFirstChild("MessageArea")
            and p1004.MessageArea:FindFirstChild("ErrorFrame") then
            task.wait(1)
            game:GetService("TeleportService"):Teleport(game.PlaceId, __localPlayer)
        end
    end)
end
